...
function Input({ className = "", ...props }: React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      {...props}
    className={`w-full rounded-xl border border-slate-200 px-4 py-3 text-[17px] text-slate-900 placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-slate-300 ${className}`}
    />
  );
}
        ...
        <nav className="hidden items-center gap-2 md:flex">
          {pages.map((item) => (
            <button
             ...
             className={`rounded-full px-5 py-2.5 text-[17px] font-medium transition ${
                page === item.id
                  ? "bg-black text-white"
  ...
  <div className="mx-auto grid max-w-7xl gap-10 px-6 py-14 md:grid-cols-3 md:px-8">
    ...
    <div className="space-y-5">
      <div className="flex items-center gap-4">
        ...
        />
        <div>
          <div className="text-lg font-semibold text-slate-900">Vidame</div>
          <div className="text-base leading-snug text-slate-600">
            {lang === "fr"
      ...
      <p className="text-base leading-7 text-slate-700">
        {lang === "fr"
          ? "Le pipeline Vidame (chaîne opératoire) est conçu pour structurer les données de recherche qualitative en vue de leur analyse."
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-4 text-base text-slate-700">
        <div className="flex items-center gap-3">
          <Mail className="h-5 w-5" /> contact@vidame.ca
        </div>
...
<h1 className="text-left text-[28px] font-[350] leading-[1.05] text-slate-900 antialiased md:mx-auto md:max-w-3xl md:text-center md:text-[48px] md:text-slate-800">
  {lang === "fr" ? (
    <>
...
<div className="mt-8 text-left text-[20px] font-[350] leading-7 text-slate-900 md:mx-auto md:max-w-lg md:text-center">
  {lang === "fr" ? (
    <>
      <p>
        Dans le contexte actuel de transformation numérique des pratiques en recherche humaine, les outils d’intelligence artificielle redéfinissent les modalités de traitement des données qualitatives.
      </p>
      ...
      <p className="mt-4 text-[20px] font-[350] leading-7 text-slate-900 md:mx-auto md:max-w-lg md:text-center">
    ...
    <>
    <p className="md:mx-auto md:max-w-2xl">
  In the current context of digital transformation in research,
  <br className="hidden md:block" />
  artificial intelligence tools are rapidly reshaping 
  <br className="hidden md:block" />
  qualitative data processing practices.
</p>
...
<div className="mx-auto mt-16 max-w-lg text-center">
  ...
  <p className="mt-4 text-[18px] leading-relaxed text-slate-500">
    {lang === "fr"
      ? "Le résultat présente un corpus segmenté en unités de sens, dans un format tabulaire (compatible Excel ou CSV), prêt pour le codage en équipe ou pour l’utilisation des logiciels CAQDAS (NVivo, MAXQDA, LibreQDA, QDA Miner). "
  ...
  <div className="max-w-3xl space-y-8">
    ...
    <div>
      <button
       ...
       className="inline-flex items-center gap-2 rounded-full border border-slate-400 bg-slate-300 px-5 py-2.5 text-[16px] font-medium text-slate-900 transition hover:bg-slate-400"
      >
        {charteOpen
          ? lang === "fr"
            ? "Réduire"
...
function Soumission({ lang = "fr" }: { lang?: "fr" | "en" }) {
  ...
  const content = {
    fr: {
      ...
      supportOptions: {
        ...
        segmentation_analytique: "Extraction du lexique et collocations",
    ...
    en: {
      ...
      supportOptions: {
        ...
        segmentation_analytique: "Lexicon and collocation extraction",
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-16 md:px-8">
      ...
      <Button type="submit" className="mt-8 rounded-full px-6 py-3 text-sm font-medium">
          {t.submit}
        </Button>
      </form>
    </div>
